...
from Model import Generator, GRUDIS
...
D = GRUDIS(input_dim=1, hidden_dim=50, output_dim=1, num_layers=2)
...
for epoch in range(opt.Epoches):
    ...
    for batch in range(int(len(pointlistx)/opt.Seq_Len/opt.BATCH_SIZE)):
        # 序列长度，batch_size,维度
        batch_noise = Variable(torch.randn(opt.Seq_Len, opt.BATCH_SIZE, 5).cuda())
...
torch.save(G.state_dict(), 'modelsaved/generatorgru.pkl')
